...
const CACHE_KEY = "project-desc-scoping-cache";
...
function getStatusTag(v) {
  ...
  if (/operational|compliant/i.test(v) && !/non/i.test(v)) return <Tag color="green" bordered={false}><CheckCircleOutlined /> {v}</Tag>;
  if (/non/i.test(v)) return <Tag color="red" bordered={false}><CloseCircleOutlined /> {v}</Tag>;
  return <Tag bordered={false}>{v}</Tag>;
}

export default function ProjectDescScoping() {
  ...
  useEffect(() => {
    ...
    api.get(`/project-desc-scoping/history/${encodeURIComponent(name)}`)
      .then(({ data }) => {
        const enriched = data.map((r) => ({ ...r, ...computeFields(r) }));
  ...
  const handleDelete = (record) => {
    Swal.fire({ title: "Delete this record?", text: `${record.municipality}, ${record.province}`, icon: "warning", showCancelButton: true, confirmButtonColor: "#ff4d4f", confirmButtonText: "Delete" })
      .then(async (result) => { if (result.isConfirmed) { await api.delete(`/project-desc-scoping/${record._id}`); secureStorage.remove(CACHE_KEY); setRecords((prev) => prev.filter((r) => r._id !== record._id)); Swal.fire("Deleted", "Record deleted", "success"); } });
  };
  ...
  const filtered = useMemo(() => {
    let data = records;
    if (searchText) { const s = searchText.toLowerCase(); data = data.filter((r) => [r.province, r.municipality, r.focalPerson, r.enmoAssigned, r.statusOfPDS, r.mailingAddress].some((v) => v && v.toLowerCase().includes(s))); }
    if (filterProvince) data = data.filter((r) => r.province === filterProvince);
    if (filterMBA) data = data.filter((r) => r.manilaBayArea === filterMBA);
    if (filterStatus) data = data.filter((r) => r.statusOfPDS === filterStatus);
  ...
  const filters = useMemo(() => ({
    ...
    statusOfPDS: buildFilters(records, "statusOfPDS"),
  ...
  const columns = [
    ...
    { title: "Status", dataIndex: "statusOfPDS", key: "status", width: 130,
      filters: filters.statusOfPDS, onFilter: (v, r) => r.statusOfPDS === v,
      render: (v) => getStatusTag(v),
    },
    {
      title: <><FileDoneOutlined style={{ color: "#2f54eb" }} /> Permits</>, key: "permits", width: 170,
      render: (_, r) => (
        <div style={{ lineHeight: 1.3 }}>
          <Text style={{ fontSize: 10 }}>TSD: {r.contractWithTSD ? <Tag color="green" bordered={false} style={{ fontSize: 10 }}>Yes</Tag> : "—"}</Text><br />
          <Text style={{ fontSize: 10 }}>Bins: {r.numberOfBinsInstalled ?? "—"} · HW Cert: {r.hazwasteGeneratorCert ? "✓" : "—"}</Text>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size={4}>
          ...
          <Tooltip title="Add Record"><Button type="text" size="small" icon={<PlusOutlined style={{ color: "#13c2c2" }} />} onClick={() => openAdd({ municipality: record.municipality, province: record.province, mailingAddress: record.mailingAddress, manilaBayArea: record.manilaBayArea })} /></Tooltip>
        </Space>
      ),
    },
  ];
  ...
  const statusCounts = useMemo(() => { const m = {}; filtered.forEach((r) => { const s = r.statusOfPDS || "Unknown"; m[s] = (m[s] || 0) + 1; }); return m; }, [filtered]);

  return (
    <div>
      <style>{`
        @keyframes fadeInUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
        .pds-card { animation: fadeInUp 0.5s ease-out both; }
        .pds-icon-bounce { display: inline-block; transition: transform 0.2s; cursor: default; }
      `}</style>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
        <Title level={4} style={{ margin: 0 }}><FileDoneOutlined /> Project Description for Scoping (PDS)</Title>
        <Space wrap>
          ...
          <Button icon={<DownloadOutlined />} onClick={() => {
            exportToExcel(filtered.map((r) => ({
              Province: r.province, Municipality: r.municipality, "Mailing Address": r.mailingAddress,
              "Facility Location": r.locationOfFacility, "Date Installation": r.dateOfInstallation,
              "MBA": r.manilaBayArea, Status: r.statusOfPDS, "Contract TSD": r.contractWithTSD,
              "Bins Installed": r.numberOfBinsInstalled, "HW Cert": r.hazwasteGeneratorCert,
        ...
        <Row gutter={[10, 10]} align="middle">
          ...
          <Col flex="auto">
            <Space wrap size={8}>
              ...
              <Select placeholder="Status" value={filterStatus} onChange={setFilterStatus} allowClear style={{ width: "100%", minWidth: 120, maxWidth: 180 }} size="small" options={filters.statusOfPDS.map((f) => ({ label: f.text, value: f.value }))} />
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
        <Col xs={24} sm={12} lg={6}>
          <Card size="small" hoverable className="pds-card" style={{ borderRadius: 10, borderLeft: "3px solid #1a3353", height: "100%" }}>
            <Statistic title="Total PDS" value={filtered.length} prefix={<FileDoneOutlined className="pds-icon-bounce" style={{ color: "#1a3353" }} />} />
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card size="small" hoverable className="pds-card" style={{ borderRadius: 10, borderLeft: "3px solid #52c41a", height: "100%", animationDelay: "0.07s" }}>
            <Statistic title="Status Breakdown" value={Object.keys(statusCounts).length} suffix="categories" prefix={<CheckCircleOutlined className="pds-icon-bounce" style={{ color: "#52c41a" }} />} />
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card size="small" hoverable className="pds-card" style={{ borderRadius: 10, borderLeft: "3px solid #1890ff", height: "100%", animationDelay: "0.14s" }}>
            <Statistic title="With TSD Contract" value={filtered.filter((r) => r.contractWithTSD).length} prefix={<SafetyCertificateOutlined className="pds-icon-bounce" style={{ color: "#1890ff" }} />} />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card size="small" hoverable className="pds-card" style={{ borderRadius: 10, borderLeft: "3px solid #faad14", height: "100%", animationDelay: "0.21s" }}>
            <Statistic title="Monitored" value={filtered.filter((r) => r.dateOfMonitoring).length} suffix={`/ ${filtered.length}`} prefix={<CalendarOutlined className="pds-icon-bounce" style={{ color: "#faad14" }} />} />
          </Card>
        </Col>
      </Row>
      ...
      <Modal title={<Space><FileTextOutlined />{detailModal?.municipality}, {detailModal?.province}{detailYearRecords.length >= 1 && <Tag color="blue" bordered={false} style={{ marginLeft: 8 }}>{detailYearRecords.length} year records</Tag>}</Space>} open={!!detailModal} onCancel={() => setDetailModal(null)} footer={<Button onClick={() => setDetailModal(null)}>Close</Button>} width={800} style={{ maxWidth: "95vw" }}>
        {detailModal && (
          ...
          <Collapse defaultActiveKey={["general","permits","monitoring","monitoring2","compliance"]} bordered={false} items={[
            { key: "general", label: <span style={{ color: "#1677ff" }}><EnvironmentOutlined /> General Info</span>, children: (
              <Descriptions column={2} size="small" bordered>
                ...
                <Descriptions.Item label="Mailing Address">{detailViewRecord.mailingAddress || "—"}</Descriptions.Item>
                <Descriptions.Item label="Facility Location">{detailViewRecord.locationOfFacility || "—"}</Descriptions.Item>
                <Descriptions.Item label="Installation Date">{detailViewRecord.dateOfInstallation || "—"}</Descriptions.Item>
                ...
                <Descriptions.Item label="Status">{getStatusTag(detailViewRecord.statusOfPDS)}</Descriptions.Item>
                <Descriptions.Item label="HW Generation">{detailViewRecord.hwGeneration || "—"}</Descriptions.Item>
              </Descriptions>
            )},
            { key: "permits", label: <span style={{ color: "#fa8c16" }}><FileDoneOutlined /> Permits</span>, children: (
              <Descriptions column={2} size="small" bordered>
                <Descriptions.Item label="Contract with TSD">{detailViewRecord.contractWithTSD || "—"}</Descriptions.Item>
                <Descriptions.Item label="Bins Installed">{detailViewRecord.numberOfBinsInstalled ?? "—"}</Descriptions.Item>
                <Descriptions.Item label="HW Generator Cert">{detailViewRecord.hazwasteGeneratorCert || "—"}</Descriptions.Item>
                <Descriptions.Item label="ECC">{detailViewRecord.ecc || "—"}</Descriptions.Item>
                <Descriptions.Item label="Wastewater Discharge">{detailViewRecord.wastewaterDischargePermit || "—"}</Descriptions.Item>
                <Descriptions.Item label="APSI Permit">{detailViewRecord.permitToOperateAPSI || "—"}</Descriptions.Item>
              </Descriptions>
            )},
      ...
      <Modal title={editing ? "Edit PDS" : "Add PDS"} open={modalOpen} onCancel={() => setModalOpen(false)} onOk={handleSave} width={900} style={{ maxWidth: "95vw" }} okText={editing ? "Update" : "Create"}>
        <Form form={form} layout="vertical" size="small">
          ...
          <Collapse defaultActiveKey={["location","permits","personnel","monitoring","compliance"]} bordered={false} items={[
            { key: "location", label: <span style={{ color: "#1677ff" }}><EnvironmentOutlined /> Location</span>, children: (
              <Row gutter={16}>
                ...
                <Col xs={24} sm={12}><Form.Item name="mailingAddress" label="Mailing Address"><Input /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="locationOfFacility" label="Facility Location"><Input /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="dateOfInstallation" label="Installation Date"><Input /></Form.Item></Col>
            ...
            { key: "permits", label: <span style={{ color: "#fa8c16" }}><FileDoneOutlined /> Permits</span>, children: (
              <Row gutter={16}>
                <Col xs={24} sm={12}><Form.Item name="contractWithTSD" label="Contract with TSD"><Input /></Form.Item></Col>
                <Col xs={12} sm={6}><Form.Item name="numberOfBinsInstalled" label="Bins Installed"><InputNumber style={{ width: "100%" }} /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="hazwasteGeneratorCert" label="HW Generator Cert"><Input /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="ecc" label="ECC"><Input /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="wastewaterDischargePermit" label="Wastewater Discharge Permit"><Input /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="permitToOperateAPSI" label="APSI Permit"><Input /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="statusOfPDS" label="Status"><Input /></Form.Item></Col>
                <Col xs={24} sm={12}><Form.Item name="hwGeneration" label="HW Generation"><Input /></Form.Item></Col>
              </Row>
            )},
