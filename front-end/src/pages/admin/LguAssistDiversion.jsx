...
import {
  ...
  PieChartOutlined, RiseOutlined, DeleteOutlined,
...
export default function LguAssistDiversion({canEdit = true, canDelete = true, isDark}) {
  ...
  const fetchRecords = useCallback(async (skipCache = false) => {
    await fetchWithCache(CACHE_KEY, () => api.get("/lgu-assist-diversion").then(({ data }) => data), {
      ttl: CACHE_TTL,
      force: skipCache,
  ...
  const handleDelete = (record) => {
    Swal.fire({ title: "Delete this record?", text: `${record.lgu}, ${record.province}`, icon: "warning", showCancelButton: true, confirmButtonColor: "#ff4d4f", confirmButtonText: "Delete" })
      .then(async (result) => { if (result.isConfirmed) { await api.delete(`/lgu-assist-diversion/${record._id}`); invalidateCache(CACHE_KEY); setRecords((prev) => prev.filter((r) => r._id !== record._id)); Swal.fire("Deleted", "Record deleted", "success"); } });
  };
