...
import {
  ...
  SafetyCertificateOutlined, FilterOutlined, ClearOutlined, SwapOutlined, DeleteOutlined,
...
export default function TransferStations({canEdit = true, canDelete = true, isDark}) {
  ...
  const fetchRecords = useCallback(async (skipCache = false) => {
    await fetchWithCache(CACHE_KEY, () => api.get("/transfer-stations").then(({ data }) => data), {
      ttl: CACHE_TTL,
      force: skipCache,
          ...
          <Collapse defaultActiveKey={["location","facility","personnel","monitoring","compliance"]} bordered={false} items={[
            ...
            { key: "facility", label: <span style={{ color: "#fa8c16" }}><SwapOutlined /> Facility Details</span>, children: (
              <Row gutter={16}>
                ...
                <Col xs={24} sm={12}><Form.Item name="dateOfHauling" label="Hauling Date"><DatePicker format="MM/DD/YYYY" style={{ width: "100%" }} /></Form.Item></Col>
