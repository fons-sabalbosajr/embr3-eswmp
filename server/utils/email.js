...
function getAcknowledgementEmailHTML(data) {
  ...
  const truckRows = (trucks) =>
    (trucks || [])
      .map(
        (t) =>
          `<tr><td style="padding:6px 12px;color:#333;font-size:13px;border-bottom:1px solid #f0f0f0;">${t.disposalTicketNo || "—"}</td><td style="padding:6px 12px;color:#333;font-size:13px;border-bottom:1px solid #f0f0f0;">${t.hauler || "—"}</td><td style="padding:6px 12px;color:#333;font-size:13px;border-bottom:1px solid #f0f0f0;">${t.plateNumber || "—"}</td><td style="padding:6px 12px;color:#333;font-size:13px;border-bottom:1px solid #f0f0f0;">${t.truckCapacity ? t.truckCapacity + " " + (t.truckCapacityUnit || "m³") : "—"}</td><td style="padding:6px 12px;color:#333;font-size:13px;border-bottom:1px solid #f0f0f0;">${t.actualVolume != null ? t.actualVolume + " " + (t.actualVolumeUnit || "tons") : "—"}</td><td style="padding:6px 12px;color:#333;font-size:13px;border-bottom:1px solid #f0f0f0;">${t.wasteType || "—"}</td></tr>`
      )
      .join("");
