/**
 * Seed: Import Transfer Station data from Excel
 * Usage: node seeds/seedTransferStations.js
 */
...
const SHEET_NAME = "TRANSFER STATION";
...
async function seed() {
  ...
  const records = rows
    ...
    .map((r) => ({
      ...
      typeOfOwnership: str(r["Type of ownership"]),
      actualWasteReceived: str(r["Actual Waste Received\r\n(tons per day)"] || r["Actual Waste Received\n(tons per day)"]),
      distanceToSLF: str(r["Distance (km) to the\r\nFinal Disposal\r\n(Sanitary Landfill)"] || r["Distance (km) to the\nFinal Disposal\n(Sanitary Landfill)"]),
  ...
  const deleted = await TransferStation.deleteMany({});
  console.log(`Cleared ${deleted.deletedCount} existing records.`);
  const inserted = await TransferStation.insertMany(records);
  console.log(`Inserted ${inserted.length} records into transfer_stations.`);
